...
/**
 * Shared dashboard body, rendered by both the org-wide "All Workspaces"
 * dashboard (app/(app)/dashboard) and a specific workspace's dashboard
 * (app/(app)/app/[workspaceSlug]/dashboard) -- same compliance engine,
 * same cards, just fed a roster already scoped (or not) to one
 * workspace by the caller. This is the "reusable dashboard components
 * controlled by configuration" piece of the multi-industry architecture
 * (docs/ARCHITECTURE.md §16); per-industry terminology/branding on top
 * of this shared shell is a later phase.
 */
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
        {CARDS.map((card) => (
          <Link
            key={card.key}
            href={card.status ? `${employeesHref}?status=${card.status}` : employeesHref}
